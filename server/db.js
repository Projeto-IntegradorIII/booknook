...
app.get('/perfil', (require, response) => {
  if (require.session.username) {
    ...
    return response.json({
      valid: true, username: require.session.username, email: require.session.email,
...
app.post('/apagar', (require, response) => {
  const sql = "DELETE FROM livros WHERE cpf_proprietario = ?";

  db.query(sql, [require.session.cpf], (err, data) => {
    if (err) {
      console.error('Erro ao excluir livros:', err);
      return response.status(500).json({ valid: false, message: 'Erro ao excluir livros' });
    }else{
      const sql2 = "DELETE FROM usuario WHERE cpf = ?";
      
      db.query(sql2, [require.session.cpf], (err, data2) => {
        if (err) {
          console.error('Erro ao excluir usuário:', err);
          return response.status(500).json({ valid: false, message: 'Erro ao excluir usuário' });
        }

        if (data2.affectedRows > 0) {
          
          return response.json({ valid: true, message: 'Conta excluída com sucesso' });
        } else {
          
          return response.status(404).json({ valid: false, message: 'Usuário não encontrado' });
        }
      });
    } 
  });
